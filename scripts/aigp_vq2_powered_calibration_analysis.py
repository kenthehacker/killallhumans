...
def build_prospective_publications(
    ...
    *,
    ...
    claim = {
        ...
        "derivative_sha256": [],
        "collision_policy": (
            "f02_fixed_future_whole_run_discovery_fit_or_global_exclusion"
        ),
    }
    claim = _validated(
...
def _validate_collision_runs(value: Any, *, anchor_run_id: str) -> list[dict[str, Any]]:
    ...
    if (
        anchor_run_id == "F02-A01/reset-epoch-1/excitation-1"
        and anchor["assigned_split"] != "discovery_fit"
    ):
        _fail("$anchor_run_id", "F02 is immutable discovery_fit")
    return checked
...
def plan_global_content_assignment(
    ...
    *,
    ...
    Cross-split components containing the immutable F02 anchor either join its
...
class ProductionPostReleaseService:
    """Ordered pure/offline verifier and record builder for the sole F02/A01 run.
