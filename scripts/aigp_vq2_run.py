...
VISUAL_POWERED_STAGES = (VISUAL_COURSE_STAGE,)
# Retired-coordinator bounds still referenced by the retained hover/braking
# envelope and capture metadata.
...
def replay_controller_envelope(stage: str) -> Dict[str, Any]:
    """Return capture-level limits, with state detail for the clean course."""
    ...
    if stage == VISUAL_COURSE_STAGE:
        ...
        return {
            ...
            "phase_envelopes": {
                "track": {
                    "single_law_visual_navigation": True,
                    "max_measured_yaw_rate_rad_s": (
                        limits.max_measured_yaw_rate_rad_s
                    ),
                },
                "predict": {
                    "short_gap_propagation_only": True,
                },
                "coast_for_credit": {
...
class VQ2Runner:
    ...
    async def _run_gate0(
        ...
        *,
        ...
        pitch_blend_s = phase_timing.gate0_pitch_blend_s
        gate0_target_pitch_rad(
            context.spawn_pitch_rad,
            exit_pitch_rad,
            0.0,
            blend_duration_s=pitch_blend_s,
        )
        if (
    ...
    async def _observe_gate1(
        ...
        *,
        ...
        """Collect a bounded exact-zero view after a proved gate-0 pass.

        Observation authority has exact-zero attitude rates. The exact
        transition thrust may be preserved so the motors do not cut during
        handoff; no Gate 1 steering or passage is supplied.
        """

        if (
            type(hold_thrust) not in {int, float}
            or not math.isfinite(float(hold_thrust))
            or float(hold_thrust) not in {0.0, 0.275}
        ):
            raise ValueError(
                "post-gate observation thrust must be exact zero or the "
                "validated continuation value"
            )
