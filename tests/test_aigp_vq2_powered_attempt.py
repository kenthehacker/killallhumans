...
LIVE_WORKTREE = (
    ...
    r"\wt-package2-f02-powered-calibration-attempt-live"
)
PREDECESSOR_EVIDENCE_ROOT = (
    r"C:\Users\John\aigp-evidence\2026-07-21-package2-import-environment-recovery"
)
...
def test_plan_hash_derivation_tick_lookup_and_immutability_are_exact():
    ...
    assert contract.EXCITATION_PLAN_ID == "vq2-build3385-training-f02-excite-v1"
    assert contract.canonical_object_sha256(plan) == contract.EXCITATION_PLAN_SHA256
    assert contract.canonical_file_sha256(plan) == (
        "3919641d802e8c4d2a8124b0c97b51bd14029fedeca397d19b09eca4d264d2be"
    )
...
def test_live_freeze_attempt_and_process_authority_cross_bind():
    ...
    assert freeze["paths"]["live_freeze"] == (
        contract.EVIDENCE_ROOT + r"\live-freeze-F02-A01.json"
    )
    ...
    for stale_path in (
        ...
        PREDECESSOR_EVIDENCE_ROOT + r"\live-freeze-F01-A01.json",
        ...
        HISTORICAL_EVIDENCE_ROOT + r"\live-freeze-F00-A01-recovery-02.json",
    ):
    ...
    for key_path, stale_value in (
        ...
        (("session", "session_id"), "F01"),
        (("session", "attempt_id"), "F01-A01"),
    ):
...
def test_bundle_verification_split_and_complete_terminal_shapes():
    ...
    claim = {
        ...
        "run_id": "F02-A01/reset-epoch-1/excitation-1",
        ...
        "derivative_sha256": [],
        "collision_policy": "f02_fixed_future_whole_run_discovery_fit_or_global_exclusion",
    }
    ...
    registry = {
        ...
        "content_groups": [{
            "decoded_sha256": H,
            "run_ids": ["F02-A01/reset-epoch-1/excitation-1"],
            "assigned_split": "discovery_fit",
