...
def test_missing_fov_deferral_is_scoped_to_current_owned_top_boundary(
    ...
    assert (
        course_stage._promoted_top_boundary_may_defer_missing_fov(
            mode=mode,
            lifecycle=lifecycle,
            recovery_measurement_mode=measurement,
            successor_handoff_required=handoff_required,
            committed_crossing=committed_crossing,
            current_clipping=clipping,
            current_center_censored=center_censored,
        )
        is expected
    )
