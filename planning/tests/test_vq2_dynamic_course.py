...
def test_unsafe_vertical_passage_keeps_current_gate_heading() -> None:
    core = DynamicCourseCore(
        DynamicCourseConfig(
            camera_delay_s=0.0,
            successor_clearance_dwell_s=0.04,
    ...
    assert qualified.command.yaw_rate_rad_s < 0.0
...
def test_trace_3ff977f_successor_flips_cannot_hunt_current_gate_yaw() -> None:
    ...
    # This replay never establishes a safe current-aperture passage.  Passage,
    # promotion, and successor steering authority therefore remain withheld.
    assert all(
    ...
    assert all(
        decision.precommit_successor_roll_authority == 0.0
        and decision.precommit_successor_target_roll_rad is None
        and decision.precommit_successor_yaw_authority == 0.0
        and decision.precommit_successor_yaw_rate_rad_s is None
        for decision in decisions
    )
    for decision in decisions:
        assert (
...
def test_successor_dropout_retains_local_rate_for_reacquisition_continuity(
    ...
    assert committed.committed_successor_roll_authority == 0.0
    assert committed.committed_successor_target_roll_rad is None
    ...
    assert expired_committed.committed_successor_roll_authority == 0.0
    assert expired_committed.committed_successor_target_roll_rad is None
...
def test_committed_fresh_off_axis_successor_turns_with_full_negative_bank(
) -> None:
    config = DynamicCourseConfig(
        camera_delay_s=0.0,
        bearing_alpha=1.0,
    ...
    assert outward.committed_successor_yaw_authority == 1.0
    assert outward.command.yaw_rate_rad_s < 0.0
    ...
    assert abs(
        recovered.committed_successor_target_roll_rad
    ) < abs(outward.committed_successor_target_roll_rad)
    assert recovered.command.target_roll_rad == pytest.approx(
        recovered.committed_successor_target_roll_rad
    )
    assert recovered.committed_successor_yaw_authority == 1.0
...
def _uncommitted_successor_case(
    *,
    ...
    for sequence in range(1, 9):
        ...
        if sequence == 1:
            core.bind(
                ...
                current_track_id="gate-a",
                successor_track_id=(
                    "gate-b" if bind_successor else None
                ),
            )
        decision_time = observation_time + 0.005
        _imu(core, decision_time)
...
@pytest.mark.parametrize("successor_x_step", (0.004, -0.004))
def test_uncommitted_successor_cannot_claim_full_precommit_yaw(
    successor_x_step: float,
) -> None:
    ...
    _, current_only = _uncommitted_successor_case(
        successor_x_step=successor_x_step,
        current_y=-0.05,
        bind_successor=False,
    )

    assert not with_successor.passage_committed
    assert with_successor.successor_prediction_confidence > 0.0
    ...
    assert with_successor.precommit_successor_yaw_heading_delta_rad is None
    assert with_successor.precommit_successor_yaw_contribution_rad is None
    assert with_successor.precommit_current_horizontal_fov_clearance_norm is None
    assert with_successor.command.target_pitch_rad == (
        current_only.command.target_pitch_rad
    )
    assert with_successor.command.thrust == current_only.command.thrust
    assert all(
        math.isfinite(value)
        for value in (
...
def test_centered_current_gate_waits_for_passage_commitment_to_turn() -> None:
    ...
    committed_time_ns = 1_300_000_000
    _imu(core, committed_time_ns / NS)
    committed = core.guide(
        committed_time_ns,
        passage_committed=True,
    )
    ...
    assert committed.committed_successor_target_roll_rad == pytest.approx(
        MAX_TARGET_ROLL_RAD
    )
    assert committed.command.target_roll_rad == pytest.approx(
    ...
    assert abs(committed.command.target_roll_rad) <= MAX_TARGET_ROLL_RAD
    assert abs(committed.command.yaw_rate_rad_s) <= MAX_YAW_RATE_RAD_S


def test_uncommitted_successor_is_withheld_without_crossing_reserve() -> None:
    _, unsafe = _uncommitted_successor_case(
        successor_x_step=0.004,
...
def test_uncommitted_successor_does_not_create_a_yaw_lease() -> None:
